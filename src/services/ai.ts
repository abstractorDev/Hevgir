...
۳. پیشنهاد (Recommendation): بر اساس تحلیل بالا، دقیقاً یک مقاله معتبر، کتاب مهندسی، یا مفهوم بنیادین علوم کامپیوتر/فلسفه برای مطالعه پیشنهاد دهید.

قوانین فرمت‌بندی (بسیار مهم):
- به هیچ وجه از هشتگ (#) یا خط جداکننده (---) استفاده نکن.
- برای بولت‌پوینت‌ها فقط و فقط از خط تیره (-) استفاده کن (از ستاره تکی * استفاده نکن).
- فقط مجاز هستی از جفت ستاره (**) برای پررنگ کردن کلمات کلیدی استفاده کنی.
  `.trim();
